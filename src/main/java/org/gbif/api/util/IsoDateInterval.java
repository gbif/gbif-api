...
public class IsoDateInterval {
  ...
  public void setFrom(String textFrom) {
    this.setFrom(parseToTemporal(textFrom));
  }
  ...
  public void setTo(String textTo) {
    this.setTo(parseToTemporal(textTo));
  }
  ...
  public static IsoDateInterval fromString(String text) throws ParseException {
    ...
    if ((index = text.indexOf("/")) >= 0) {
      if (index + 1 < text.length()) {
        // Explicit range
        return new IsoDateInterval(parseToTemporal(text.substring(0, index)), parseToTemporal(text.substring(index + 1)));
      } else {
        throw new ParseException("Missing 'to' end of IsoDateInterval in " + text, index);
      }
    } else {
      return new IsoDateInterval(parseToTemporal(text));
    }
  }
  ...
  public static IsoDateInterval fromString(String textFrom, String textTo) throws ParseException {
    return new IsoDateInterval(parseToTemporal(textFrom), parseToTemporal(textTo));
  }

  /**
   * Parses the ISO 8601-format date range parts generated by toString().
   */
  public static Temporal parseToTemporal(String text) {
